...
// Footer auto-appended to every comment body posted through this server, so
// downstream readers can tell at a glance that the comment is AI-generated.
// GitHub's author chip still reflects the token owner — see README for why.
const CLAUDE_FOOTER =
  "\n\n---\n🤖 Generated by Claude Code via the [github-pr-review MCP](https://github.com/SalixDev/mcp-github-pr-review).";

function withAttribution(body: string): string {
  return body.includes(CLAUDE_FOOTER) ? body : body + CLAUDE_FOOTER;
}
  ...
  try {
    ...
    if (name === "add_pr_comment") {
      ...
      const { data } = await octokit.issues.createComment({
        ...
        body: withAttribution(args.body),
      });
      return {
        content: [
